...
export class PlannerAgent {
  ...
  private async llmPlan(operations: Operation[], spec: NormalizedSpec, baseUrl: string): Promise<AgentTestPlan> {
    ...
    const prompt = `You are an expert API test architect. Analyze this OpenAPI specification and produce a test plan for POSITIVE (happy path) scenarios.
...
7. Return ONLY valid JSON. No markdown, no explanation outside the JSON.`;
    ...
    const response = await this.llm.generate({
      prompt,
      systemPrompt: 'You are an API testing expert. Return only valid JSON. No markdown fences.',
      temperature: 0.2,
      maxTokens: 6000,
    });
